...
pub struct Data {
    ...
    pub entitlement_cache:
        mini_moka::sync::Cache<UserId, (PremiumInfo, Option<serenity::Timestamp>)>,
...
impl Data {
    ...
    fn get_cached_discord_premium(&self, user_id: &UserId) -> Option<PremiumInfo> {
        if let Some((premium_info, expiry_time)) = self.entitlement_cache.get(user_id) {
            if expiry_time.is_none_or(timestamp_in_future) {
                return Some(premium_info);
            }

            self.entitlement_cache.invalidate(user_id);
        }
    ...
    async fn fetch_entitlement_for_user(
            ...
            .find(|entitlement| entitlement.ends_at.is_none_or(timestamp_in_future))
        {
            ...
            self.entitlement_cache
                .insert(user_id, (premium_info, entitlement.ends_at));
            Ok(Some(premium_info))
        } else {
            Ok(None)
        }
    }
